...
function toCallBooked({ contact }: RunContext) {
  ...
  return { effect: { opportunity: { stage: 'Call Booked' } }, log: 'Enrollment › Call Booked, moved from Applied.' };
}

/** Add Task with Due In 1 day, Due Time 11:00 AM and Skip Weekends on. */
function callTask({ contact, now }: RunContext) {
  const canText = contact.fields.application_sms_consent === 'Yes' && !contact.dnd.sms;
  return {
    log: `Task for Devon Brooks, due ${formatClock(nextWeekdayAt(now, 11 * 60))}: reach ${contact.firstName} ${contact.lastName} about the coaching application (score ${contact.fields.application_score}). No call booked in 2 days. ${canText ? 'Call or text.' : 'Call or email only: no text consent.'}`,
  };
}
...
const answers = (a: Answers): Contact['fields'] => ({
  ...
  pivot_story: a.story,
  application_sms_consent: a.texts,
});

/** A workshop registrant as 01 left them: an open Enrollment card, no owner. */
const registrant = (stage: string, tags: string[], a: Answers, extra: Contact['fields'] = {}): Partial<Contact> => ({
  tags,
  opportunity: { pipeline: 'Enrollment', stage, status: 'open', name: 'Marcus Lee · Workshop' },
  fields: { sms_consent: 'Yes', sms_marketing_consent: 'No', ...extra, ...answers(a) },
});

/** A student as 03 left them: the course card at Customer and Won, Jules as owner. */
const student = (tags: string[], a: Answers): Partial<Contact> => ({
  tags,
  assignedTo: 'jules',
  opportunity: { pipeline: 'Enrollment', stage: 'Customer', status: 'won', value: 497, name: 'Marcus Lee · Career Pivot Blueprint' },
  fields: { purchase: 'Career Pivot Blueprint', payment_plan: 'No', sms_consent: 'No', sms_marketing_consent: 'No', ...answers(a) },
});

/* ---------- Copy ---------- */

const devonSign = '{{user.name}}\nEnrollment advisor, {{location.name}}';
...
It is 30 minutes on video. We talk about where you are now, the change you want to make and the time you can give it, and I explain how the coaching works and what it costs. If the course or the free workshop would serve you better right now, I will tell you that too. It is a conversation, not a pitch.

Pick a time that suits you: {{custom_values.call_booking_link}}

You get a calendar invite and a reminder the day before.

${devonSign}`;

const followUpBody = `Hi {{contact.first_name}},

I'm following up on your coaching application myself. I don't see a strategy call on the calendar yet, and that's fine. Weeks get busy.

If you still want to talk it through, any open time is here: {{custom_values.call_booking_link}}

If none of those times work, reply with two or three that do and I'll set it up for you. And if you have decided now isn't the right time, just say so and I'll close out your application. You can apply again whenever you're ready.
...
You already have {{custom_values.course_name}}, and based on your answers, the most useful next step is to keep going with it rather than add coaching right now. Work through the skills inventory and the 90-day plan first. If you then want someone in your corner for the move itself, apply again and your new answers are looked at fresh: {{trigger_link.apply}}

Pick up where you left off: {{custom_values.course_login}}
...
const setupSheet = `Coaching Application          Sites › Surveys
  Layout        One question per slide, progress bar on, Sticky Contact on
  Questions     Name, email, phone        standard fields
                ...
                Texts about my application  Application Texts, unticked, optional
  Conditional Logic (v2), two rules on the same condition:
                Help Wanted is "A recruiter to find me a job"
                  Disqualify Lead
                  Display Custom Message: "We coach people through a career
                  change. We don't place people in jobs, and nobody can promise
                  a job offer. The free workshop is a good place to start: ..."
  On submit     Message: "Thanks for applying. Check your inbox in the next
                few minutes for your next step."
  Snapshots     Conditional Logic rules are not included in snapshots.
                Rebuild both rules in any account this survey is copied to.
...
05a · Sales · Strategy Call Booked
  Trigger       Appointment Status
                  In Calendar: Strategy Call
                  Appointment Status: New
  Action        Add Contact Tag: call-booked
  Why this      Appointment Status also fires when Devon books the call for
  trigger       them. Customer Booked Appointment only fires when the contact
                books it themselves.
...
export const application: Automation = {
  ...
  solution:
    'The survey handles the one hard no itself, with a Disqualify rule, and the trigger’s Disqualified filter keeps those submissions out. Everything else is scored 0 to 100 by a Custom Code step from four answers. At 70 or more the card moves to Applied, Devon becomes the owner and gets the answers by email, and the applicant gets the booking link by email and, with consent, by text. A booked Strategy Call moves the card to Call Booked. No booking in two days gives Devon a task and sends a personal follow-up. Under 70, the applicant gets a kind email that recommends the course first, or tells a current student to keep going, and says how to apply again.',
  workflow: {
    ...
    settings: {
      ...
      notes: [
        'Allow Re-entry on: someone who is "not a fit yet" can apply again after the course, and the new answers are scored fresh. GHL does not let a contact re-enter while still active, so a double submit during the follow-up starts nothing.',
        'Stop on Response off: a reply is usually a question about the call ("is this a sales pitch?"), and the booking still has to be tracked afterwards. Replies land in Devon’s Conversations inbox because he owns the contact. STOP still switches on SMS DND by itself.',
        'Timezone: Contact. The texting-hours and weekday-hours windows use the applicant’s time zone. A contact with no time zone falls back to the account’s, Central.',
        ...
        'Sender Details: From Name Devon Brooks, from devon@trailheadcareers.example. The two "not yet" emails set From Name Morgan Hale and morgan@trailheadcareers.example at the step, because they are not a sales conversation.',
      ],
    },
    steps: [
      {
      ...
      },
      {
        ...
        summary: 'Application Score = the code output, so the If/Else, the notification and Smart Lists all read the same number.',
            ...
            nodes: [
              {
                ...
                label: 'Devon',
                summary:
                  'Devon Brooks, the enrollment advisor. Only Apply to Unassigned Contacts is off on purpose: a current student owned by Jules moves to Devon for the sales conversation. It runs before the card and the emails, so the card gets him as owner and the emails come from him.',
                run: ({ contact }) =>
                    ...
                    nodes: [
                      {
                        ...
                        label: 'Applied',
                        summary:
                          'Enrollment › Applied on the card Find Opportunity picked, renamed "{contact name} · 1:1 Pivot Coaching", Opportunity Value 0 until Devon agrees a package on the call. Allow Opportunity to Move to Any Previous Stage stays off.',
                        run: toApplied,
                      },
                      {
                        id: 'notify',
                      ...
                      },
                      {
                        ...
                        summary: 'Straight away, from Devon. What the call is and is not, the booking link, and no promise about outcomes.',
                        message: {
                          channel: 'email',
                      ...
                      },
                      {
                        ...
                        summary: "No delay, but its Advance Window only resumes between 8 AM and 8 PM in the contact's time zone, so a 10 PM applicant gets the text at 8 AM. It sits before the consent check, so a booking made overnight is seen before any text goes out.",
                      },
                      {
                        id: 'can-text',
                        kind: 'ifelse',
                        ...
                        branches: [
                          {
                            ...
                            when: {
                              type: 'all',
                              label: 'Application Texts is Yes, not DND for SMS, and Contact Tag does not include call-booked',
                              of: [
                                { type: 'field', key: 'application_sms_consent', op: 'eq', value: 'Yes' },
                            ...
                            nodes: [
                              {
                                ...
                                title: 'Send SMS',
                                label: 'Booking link',
                                summary: 'The first text this workflow sends, so it names the business and carries the opt-out line. One link, from Devon.',
                                message: {
                                  channel: 'sms',
                                    ...
                                    nodes: [
                                      {
                                        ...
                                        title: 'Update Opportunity',
                                        label: 'Call Booked',
                                        summary: 'Enrollment › Call Booked, on the same card. Confirmation and reminders come from the Strategy Call calendar’s own notifications, so this workflow sends nothing more.',
                                        run: toCallBooked,
                                      },
                                    ],
                                  },
                                    ...
                                    nodes: [
                                      {
                                        ...
                                        summary: "No delay, but the Advance Window only resumes Monday to Friday, 9 AM to 5 PM in the contact's time zone. A personal email from Devon should not arrive at 11 PM on a Sunday.",
                                      },
                                      {
                                        id: 'task-devon',
                                        kind: 'action',
                                        ...
                                        title: 'Add Task',
                                        label: 'Reach out',
                                        summary: 'Assign To Devon Brooks, Due In 1 day at 11:00 AM, Skip Weekends on. The description carries the score and says whether texting is allowed.',
                                        run: callTask,
                                      },
                                      {
                                        id: 'email-followup',
                                        ...
                                        summary: 'Plain text from Devon, no button. It says only what is true (no call on the calendar yet) and makes "not now" an easy answer.',
                                        message: {
                                          channel: 'email',
                                            ...
                                            nodes: [
                                              {
                                                ...
                                                title: 'Go To',
                                                target: 'opp-booked',
                                                summary: 'The same Update Opportunity step as a booking in the first two days.',
                                              },
                                            ],
                                          },
                                          timeout: {
                          ...
                          nodes: [
                            {
                              ...
                              title: 'Go To',
                              target: 'wait-book',
                              summary: 'No text consent, SMS DND, or a call already booked: straight to the booking wait. From here the path is the same, minus the text.',
                            },
                          ],
                        },
                      },
                  ...
                  nodes: [
                    {
                      ...
                      label: 'Applied',
                      summary:
                        'Enrollment › Applied, named "{contact name} · 1:1 Pivot Coaching", value 0, Duplicate Opportunity on. Only reached when there is no open card: no card at all, or a closed one such as a student’s Won course sale, which stays as it is.',
                      run: ({ contact }) => {
                  ...
                  nodes: [
                    {
                      ...
                      title: 'Send Email',
                      label: 'Keep going',
                      summary: 'From Morgan. No pitch for a course they already own: their login, the next two milestones and how to apply again.',
                      message: {
                        channel: 'email',
  ...
  scenarios: [
    {
      ...
      expect: {
        outcome: 'completed',
        visits: ['clear-tags', 'if-score:0', 'assign', 'find-opp:0', 'opp-applied', 'notify', 'email-invite', 'quiet', 'can-text:0', 'sms-invite', 'wait-book:met', 'opp-booked'],
        stage: 'Call Booked',
      },
    },
    {
      ...
      summary: 'Applies at 10:10 PM on a Friday with texts ticked. The text waits for 8 AM. Asks about price on Saturday, never books, and gets Devon’s follow-up on Monday.',
      start: at(4, 22, 10),
      contact: registrant('Attended', ['workshop-replay'], {
      ...
      expect: {
        outcome: 'completed',
        visits: ['quiet', 'can-text:0', 'sms-invite', 'wait-book:timeout', 'office-hours', 'task-devon', 'email-followup', 'wait-late:timeout', 'tag-no-call'],
    ...
    },
    {
      ...
      label: 'Student applies again, books late',
      summary: 'Got "not yet" in January, bought the course, and applies again on a Sunday afternoon without ticking texts. Books the morning after Devon’s follow-up email.',
      start: at(6, 15, 20),
      ...
      events: [
        {
          at: at(9, 10, 5) - at(6, 15, 20),
          type: 'appointment_booked',
          value: 'Strategy Call',
          appointmentAt: at(11, 13) - at(6, 15, 20),
          label: 'Strategy Call, Friday 1:00 PM, from the link in the follow-up email',
        },
      ],
      expect: {
        outcome: 'completed',
        visits: ['clear-tags', 'assign', 'find-opp:else', 'create-opp', 'goto-find', 'find-opp:0', 'opp-applied', 'can-text:else', 'goto-book', 'wait-book:timeout', 'task-devon', 'email-followup', 'wait-late:met', 'goto-booked', 'opp-booked'],
        stage: 'Call Booked',
      },
    },
    {
      ...
      expect: {
        outcome: 'completed',
        visits: ['if-score:else', 'tag-not-yet', 'is-student:0', 'email-student'],
        tags: ['not-a-fit-yet'],
        stage: 'Customer',
      },
    },
  ...
  dataModel: {
    customFields: [
      ...
      { name: 'Application Texts', key: 'application_sms_consent', type: 'Checkbox', note: 'The survey’s own box, unticked and optional. A text about a sales call needs its own consent' },
      { name: 'Application Score', key: 'application_score', type: 'Number', note: 'Written by the Custom Code step, 0 to 100' },
    ],
    tags: [
      { name: 'not-a-fit-yet', note: 'Scored under 70 and got the "not yet" email. Cleared when they apply again' },
      { name: 'call-booked', note: 'Added by 05a when a Strategy Call is created. The waits read it, and each new application clears it' },
  ...
  build: [
    ...
    },
    {
      ...
      body: 'Four properties go in through inputData and the score comes out through output. GHL only makes a Custom Code output available to later steps after a successful test, so I tested it with sample answers that should land at 65, 70 and 75, and with a capped set. Update Contact Field writes the score to Application Score, and the If/Else reads that field, so the number Devon sees and the number that routed the applicant are the same number.',
    },
    {
      title: 'Find the card before moving it',
      body: 'Assign To User runs first, so any new card gets Devon as owner. Find Opportunity looks for the latest open card in Enrollment, which a workshop registrant already has, and Update Opportunity moves it to Applied. A student’s course card is Won, not open, so Find comes back empty and Create Opportunity makes a second card, with Duplicate Opportunity on and Allow Multiple Opportunities per Contact switched on in Settings › Objects › Opportunities. Then a Go To runs Find again, because GHL does not carry a created card into later Update Opportunity steps.',
    },
    {
      title: 'Invite by email, text only with consent',
      body: 'The email goes out at once, from Devon. The text waits for an 8 AM to 8 PM Advance Window, then checks the survey’s own Application Texts box, SMS DND and whether a call is already booked. The workshop-reminder box does not count: a text about a sales call is marketing and needs its own consent. The Strategy Call calendar sends the confirmation and reminder by email from its Notifications tab; its SMS notifications stay off, because they do not read the consent field.',
    },
    {
      title: 'Watch for the booking with a helper',
      body: 'A Wait can hold until a time relative to an appointment that already exists, but it cannot wait for one to be booked, and the If/Else appointment filters only appear in appointment-triggered workflows. So a one-step helper, 05a · Sales · Strategy Call Booked, adds call-booked on Appointment Status (Strategy Call, status New), which also fires when Devon books the call for someone by hand. The waits here are Specific conditions to be met on that tag: 2 days, then a task and a personal email from Devon inside weekday hours, then 7 more days so a late booking still moves the card.',
    },
    {
      title: 'Test, publish, hand off',
      body: 'Five test contacts, one per scenario above, plus one that picks the recruiter answer, checked against Execution Logs, Enrollment History and the pipeline board. Then a Loom for Devon: where applications show up, what the score means, and how to overrule it. If he thinks a "not yet" deserves a call, he books it himself, and 05a moves nothing he has to fix.',
    },
  ],
  edgeCases: [
    {
      title: 'Wants a recruiter, not a coach',
      body: 'The survey’s Disqualify Lead rule marks the submission disqualified, and a Display Custom Message rule on the same answer explains that Trailhead does not place people in jobs and points to the free workshop. The trigger’s Disqualified is false filter keeps them out of 05, so there is no card, no owner and no sales email.',
    },
    {
    ...
    },
    {
      ...
      body: 'The invitation email and Devon’s notification go out straight away. The text waits for 8 AM in their time zone, and if they booked from the email overnight, the call-booked tag is already there, so no "book your call" text goes out. The two-day follow-up only lands between 9 AM and 5 PM on a weekday.',
    },
    {
      title: 'Books by message, not by the link',
      body: 'They reply "can we do Tuesday at 4?" and Devon books it from the calendar. Customer Booked Appointment would miss that, because it only fires when the contact books. Appointment Status with status New fires for any booking, so 05a still tags them and the card still moves to Call Booked.',
    },
    {
      title: 'A current student applies',
      body: 'Their course card is Won, so Find Opportunity finds no open card, and Create Opportunity adds a second card for the coaching deal. The course sale stays Won in revenue reports. Ownership moves from Jules to Devon for the sales conversation. If they are not a fit yet, they get "keep going with the course", not an offer for a course they own.',
    },
    {
      title: 'Applies twice',
      body: 'A second submission while the first run is still going starts nothing: GHL does not re-enroll an active contact. After the run ends, Allow Re-entry lets a new application in, the first step clears last time’s tags, and the answers are scored fresh. A card already at Call Booked stays there, because backward moves are off.',
    },
  ],
  qa: [
    ...
    'Pick the recruiter answer: the survey shows the message, the submission is marked disqualified, and Enrollment History for 05 has no entry',
    'Qualified with Application Texts ticked at 2 PM: card in Applied named "... · 1:1 Pivot Coaching" with Devon as owner, notification email with every answer, invitation email, and a text with the opt-out line that stays within two segments with an 11-letter first name and the real booking URL',
    'Qualified at 10 PM: the text shows as waiting in Execution Logs and sends at 8 AM',
    'Book through the link, then have Devon book another test contact by hand: both get call-booked from 05a and both cards move to Call Booked',
    'Do not book: two days later, inside weekday hours, Devon has a task due the next weekday at 11 AM and the follow-up email is in Conversations. After 9 days, applied-no-call is on the contact',
    'Student test contact: two cards in Enrollment, the course card still Won in Customer and the new one in Applied, and the run passes through Find only twice',
    'Every merge field renders in Gmail, Outlook and on a phone, and Morgan signs off all four emails for tone and for no outcome or income claims',
  ],
  snippets: [
    ...
    {
      title: 'Application Texts checkbox',
      language: 'text',
      code: 'Text me about my application and my strategy call from Trailhead Career Coaching. Message frequency varies. Msg & data rates may apply. Reply HELP for help, STOP to opt out. Consent is not a condition of purchase.',
      note: 'Unticked by default and not required to submit the survey. The same wording goes in the A2P campaign registration.',
    },
  ],
  features: [
    'Survey Submitted',
    'Survey Conditional Logic · Disqualify Lead',
    'Remove Contact Tag',
    'Custom Code',
