...
Default sender number: the main line, (312) 555-0142
  ...
  Hi {{contact.first_name}}, it's Jordan at Harbor & Pine Roofing. Thank you
  for trusting us with your roof. Would you share an honest review of the job?
...
3  Phone            Required, so the answer lands on the existing contact
...
export const reviewsReferrals: Automation = {
  ...
  solution:
    'When a card reaches Job Complete, every customer gets the same review request the next day by text, and one email reminder if they have not clicked. A few days later a two-question check-in asks how the job went. A score of 3 or lower puts Jordan on the phone within a business day; a 4 or 5, or no answer, gets a referral email ten days later. The review request never depends on the score: it has gone to everyone before the check-in is even sent.',
  evidence: {
  ...
  workflow: {
    ...
    settings: {
      ...
      notes: [
        ...
        'Time Window Monday to Saturday, 10 AM to 7 PM, contact time zone. It holds the review requests and the emails; the alert and the task for Jordan go out the moment a low score comes in.',
        'Allow Re-entry off: one review request per homeowner, even if a card is moved back to Job Scheduled and forward again.',
        'Sender Details: From Name Jordan Blake, From Email jordan@harborpine.example. The text request uses the default sender number set in Reputation › Settings, the main line.',
      ],
    },
    steps: [
      {
      ...
      },
      {
        ...
        label: 'By text',
        summary: 'Review Type SMS. The same words and the same Google link for every customer, whatever anyone thinks of the job. The copy is the Live template in Reputation › Settings.',
        message: {
          channel: 'sms',
          body: "Hi {{contact.first_name}}, it's Jordan at Harbor & Pine Roofing. Thank you for trusting us with your roof. Would you share an honest review of the job? It helps other homeowners: {{custom_values.review_link}} Reply STOP to opt out.",
        },
      },
      { id: 'wait-3d', kind: 'wait', title: 'Wait', mode: 'time', minutes: 3 * DAY, summary: 'Three days.' },
      {
        ...
        summary: 'Review Type Email. The only reminder, on a different channel, with the same link. The Goal Event below skips it for anyone who already clicked.',
        message: {
          channel: 'email',
          subject: 'Your review of Harbor & Pine Roofing',
      ...
      },
      {
        ...
        title: 'Send Email',
        label: 'Check-in',
        summary: 'From Jordan: a two-question survey, a 1 to 5 rating and "anything we should fix?". It goes out after the review request, so no answer can decide who is asked for a review.',
        message: {
          channel: 'email',
          subject: 'How did we do on your roof?',
          body: "Hi {{contact.first_name}},\n\nNow that the crew has been gone a few days, I'd like to know how the job went. Two questions, under a minute: {{location.website}}/how-did-we-do\n\nOne asks you to rate us from 1 to 5, the other asks if anything needs fixing. I read them all. If something is not right, you can also reply to this email or call me at {{custom_values.office_phone}}, and we will put it right. Your warranty details are here: {{custom_values.warranty_link}}\n\n{{custom_values.owner_name}}\nOwner, Harbor & Pine Roofing",
        },
      },
      {
        id: 'wait-survey',
        ...
        minutes: 3 * DAY,
        summary: "Specific conditions to be met: Satisfaction is not empty. The survey's rating element writes it. Timeout: 3 days.",
        branches: {
          met: {
            label: 'Answered',
            nodes: [
              {
                ...
                branches: [
                  {
                    label: '3 or lower',
                    when: { type: 'var', key: 'survey_score', op: 'lte', value: 3, label: 'Satisfaction is 3 or lower' },
                    nodes: [
                      {
                        id: 'notify-owner',
                        kind: 'action',
                      ...
                      },
                      {
                        ...
                        summary: "For Jordan's open-issues Smart List. This path ends here, so no referral email goes out; the review request already has.",
                  ...
                  nodes: [
                    {
                      ...
                      mode: 'time',
                      minutes: 10 * DAY,
                      summary: 'Ten days, so the referral ask comes two to three weeks after the job and well clear of the review request.',
                    },
                    {
                      id: 'email-referral',
                      kind: 'action',
                      ...
                      title: 'Send Email',
                      label: 'Referral ask',
                      summary: 'A thank-you for introductions, with the referral form and the reward. It never mentions reviews: a reward next to a review ask reads as paying for reviews.',
                      message: {
                        channel: 'email',
            ...
            nodes: [
              {
                ...
                kind: 'goto',
                title: 'Go To',
                target: 'wait-referral',
                summary: 'No answer is not a complaint, so they get the referral email on the same schedule as a 4 or 5. One referral path to maintain, not two.',
              },
            ],
          },
        },
  ...
  scenarios: [
    {
      ...
      summary: 'Sam signs off at 4:30 PM on Saturday. The text waits for Monday morning; Dana replies, clicks the link, skips the reminder and rates the job 5.',
      start: S.saturday,
      ...
      expect: { outcome: 'goal', visits: ['ask-sms', 'goal-clicked', 'email-checkin', 'wait-survey:met', 'score:else', 'email-referral'], stage: 'Job Complete' },
    },
    {
      id: 'quiet',
      label: 'Never clicks, never answers',
      summary: 'A storm-damage roof for one of Luis\'s customers. Gets the text, the one email reminder and the check-in, answers none of them, and still gets the referral email.',
      start: S.quiet,
    ...
    {
      id: 'low-score',
      label: 'Rates the job 2 out of 5',
      summary: 'Clicks the review link the evening it arrives, like anyone else, then rates the job 2 with a complaint about a gutter. Jordan gets the alert and a call task; the review request is not touched.',
      start: S.low,
    ...
    },
    {
      ...
      summary: 'Replied STOP to an estimate follow-up text before she signed. The text request is skipped, and the email request is her ask: day four is a Sunday, so GHL holds it until Monday at 10 AM. She rates the job 4.',
      start: S.textsOff,
      ...
      expect: { outcome: 'completed', visits: ['ask-email', 'goal-clicked', 'email-checkin', 'wait-survey:met', 'score:else', 'email-referral'], skips: ['ask-sms'], stage: 'Job Complete' },
    },
  ],
  dataModel: {
    customFields: [
      { name: 'Satisfaction', key: 'satisfaction', type: 'Number', note: "Written by the check-in survey's rating element, 1 to 5, stored as Absolute Value. Read by the Wait and the If/Else" },
      { name: 'Job Date', key: 'job_date', type: 'Date', note: 'Set by 05 when the crew signs off. Quoted in Jordan\'s alert, so he knows how old the job is before he calls' },
    ],
    tags: [{ name: 'service-recovery', note: 'Check-in score of 3 or lower. Jordan owns it until the fix is agreed; no referral email' }],
  ...
  build: [
    ...
    },
    {
      ...
      body: 'The Review Link is a Custom Link that opens the Google review form directly, the same URL as the review_link custom value. The SMS and email request templates are written and set Live, with the main line as the default sender number. When to send after check-in is Immediately, and I set the retries so each request sends once: the workflow owns the timing, so the Time Window, the reminder and the Execution Logs are in one place.',
    },
    {
    ...
    },
    {
      ...
      body: 'Stop on Response off, because a thank-you reply is not a reason to stop. Time Window Monday to Saturday, 10 AM to 7 PM in the contact\'s time zone, well inside the 8 AM to 8 PM texting rule and closed on Sunday. Send Review Request is filed with GHL\'s communication actions, so the window holds it like a text, and the Saturday test below confirms it. Re-entry off, so nobody is asked twice.',
    },
    {
  ...
  edgeCases: [
    ...
    },
    {
      ...
      body: 'A customer on SMS DND has the text request skipped and logged, and the email request on day four is their ask, with the same link. The check-in and the referral ask are emails anyway. I chose that over a DND If/Else at the top: the review request stays on the one path every customer walks, so it can never end up on only one side of a branch.',
    },
    {
      title: 'Finished on a Saturday',
      body: 'Day one lands on Sunday, outside the Time Window, so GHL holds the text until Monday at 10 AM. The waits after it count from when it actually sent. Jordan\'s alert and task are internal and are never held.',
    },
    {
      title: 'They reply to the review text',
      body: 'Stop on Response is off, so "thanks, the crew was great" lands in Conversations and they stay in for the check-in. A reply of STOP switches SMS DND on automatically; nothing later in this workflow is a text.',
    },
    {
      title: 'Card moved back and forward',
      body: 'Re-entry is off, so a card dragged back to Job Scheduled and forward again does not ask twice. The trade-off is a repeat customer next year: the workflow does not ask again, and Jordan sends that request by hand from the contact record if he wants one.',
    },
  ],
  qa: [
    'Move a test card to Job Complete at 3 PM on a weekday: the text request arrives at 3 PM the next day and shows in Reputation › Requests as sent',
    ...
    'Submit the survey with 2 stars: Jordan gets the alert and a task due the next business day, the tag is added, no referral email follows, and nothing about the review request changes',
    'Submit with 5, and on a second contact not at all: both get the referral email ten days later',
